...
def single_prediction(predictor, scaler, cv, text_input):
    # 1. Exact text cleaning used in your training
    stemmer = PorterStemmer()
    review = re.sub("[^a-zA-Z]", " ", text_input).lower().split()
    review = [stemmer.stem(word) for word in review if not word in STOPWORDS]
    clean_text = " ".join(review)
    
    # 2. Convert to numbers and scale
    X_prediction = cv.transform([clean_text]).toarray()
    X_prediction_scl = scaler.transform(X_prediction)
    
    # 3. Get the probabilities
    probabilities = predictor.predict_proba(X_prediction_scl)
    
    # probabilities[0][1] grabs the exact percentage for Class 1 (Positive)
    prob_positive = probabilities[0][1]
    
    # 4. The Custom Threshold Hack (0.90)
    # If the model is less than 90% sure it's a good review, we label it Negative
